...
Vector Search 2.0 supports hybrid search combining dense and sparse vectors. Your Collection schema must include both dense and sparse vector fields.

```python
# Dense embedding (standard)
dense_embedding = embeddings.embed_query("machine learning")

# Sparse embedding (e.g., from BM25 or learned sparse encoder)
sparse_embedding = {
    "values": [0.9, 0.3, 0.7],  # Sparse values
    "indices": [3, 20, 45]       # Corresponding indices
}
...
results = vector_store.similarity_search_by_vector_with_score(
    ...
    k=5,
    rrf_ranking_alpha=0.7  # 70% dense, 30% sparse
)

# Results include both scores
for doc, scores in results:
    print(f"Dense score: {scores['dense_score']}")
    print(f"Sparse score: {scores['sparse_score']}")
    print(f"Content: {doc.page_content}\n")
```

**RRF ranking alpha parameter:**
- `1.0`: Only dense search
- `0.0`: Only sparse search
- `0.5`: Equal weighting
- `0.7`: 70% dense, 30% sparse

See the [hybrid search documentation](https://cloud.google.com/vertex-ai/docs/vector-search/about-hybrid-search) for more information.
